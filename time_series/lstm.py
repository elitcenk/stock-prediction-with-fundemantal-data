...
def create_model(x_train, y_train):
    ...
    model.fit(x_train, y_train, epochs=10, batch_size=1, verbose=2, callbacks=[stop_here_please])
    return model
...
for i in range(60, inputs.shape[0]):
    ...
    if i == inputs.shape[0] - 1:
        today = inputs[i + 1 - 60:i + 1, :]
        today = np.reshape(np.array(today, dtype='float64'), (1, 60, x_train.shape[2]))
...
tomorrow_price = model.predict(today)
today = scaler.inverse_transform(today[0][59])
print('Today Close : ' + str(today[0]))
print('Today High : ' + str(today[1]))
print('Today Low : ' + str(today[2]))
tomorrow_price = scaler.inverse_transform(tomorrow_price[0])
print('Tomorrow Close : ' + str(tomorrow_price[0]))
print('Tomorrow High : ' + str(tomorrow_price[1]))
print('Tomorrow Low: ' + str(tomorrow_price[2]))
